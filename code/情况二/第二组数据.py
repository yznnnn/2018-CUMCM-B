...
class PRODUCT:
    ...
    #显示该物料的资料
    def print_data(self):
        st='\t'+str(self.num)+'\t'+str(self.CNC_num_process_B1)+'\t'+str(self.start_time_process_B1)+'\t'+str(self.finish_time_process_B1)+'\t'
...
class PROJECT:
    ...
    def priority_generate(self,robot_position):
        ...
        #self.priority=a*(self.wait_time)-b*self.move_time

        #self.priority=a*(self.wait_time)+b*(1/(self.move_time+1))
        #self.priority=(self.wait_time+self.work_time)/self.work_time  #250
...
def main():
    ...
    complete_num=0
    #显示结果
    print('\t'+"加工物料序号"+'\t'+'1道工序加工CNC编号'+'\t'+'1道工序上料开始时间'+'\t'+'1道工序下料开始时间'+'\t'+'2道工序加工CNC编号'+'\t'+'2道工序上料开始时间'+'\t'+'2道工序下料开始时间'+'\t')
